...
- **三层管道**：`services/categorizer/engine.py::categorize_transaction` 返回 `MatchResult(matched, requires_llm)`；`services/ingestion/__init__.py` 在 L1 后**可选**追加 L2（after-commit 入 `services/llm/queue.py` 的限速队列）。L2 实现：`services/llm/classifier.py::classify_with_llm` —— 检索 top-N 知识库条目（按 token 重合度）→ 构造 prompt → Provider 调用 → 解析 JSON → 写 `tx.categorization_method/confidence/llm_reason` 或在 inbox stash `metadata.llm_suggestion`。
- **L2 触发（2026-07 改为手动）**：L2 **默认不再自动触发**。`app_settings.llm_auto_classify`（默认 `false`）控制导入后是否自动派遣；关闭时导入只跑 L1 关键词匹配，未命中的行留在 inbox 等用户手动处理。手动入口 `POST /api/v1/llm/classify-inbox`（把所有 `is_pending` 的 income/expense 行入队），前端在「待确认」页顶部的 **✨ AI 智能处理** 按钮触发；进度复用 `LlmQueueIndicator`（轮询 `/llm/queue`），队列排空后自动刷新 inbox 显示推荐。设置页「导入后自动 AI 分类」开关可切回自动。
