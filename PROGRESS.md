...
| 2026-05-05 | **IBAN 内部转账识别 + 余额校准 UX** | accounts 加 `iban` 字段（idempotent ALTER）+ AccountForm 加 IBAN 输入；transfer_matcher 加 IBAN 评分（+40 高权重，可独立把 amount-equal 50 → ≥75 自动配对）；Revolut parser 把 "Reference / From / To / IBAN" 续行 append 到 raw_description（IBAN 信息进入字段）；matcher 同时检索 description + raw_description；balance dialog 文案「存入/取款」→「余额操作」+「校准到目标值」模式强调用途为修正错记 |
| 2026-05-05 | **inbox 自动通过 + 扩散学习** | ① 高置信度匹配的 tx 直接 `is_pending=False` 跳过 inbox（含 categorizer 命中规则的 + 所有 transfer 类）；Revolut 39 笔 → 31 自动 + 6 transfer 自动，inbox 仅 2 笔真未匹配。② 新增 `apply_to_similar_pending`：用户对 inbox 一笔分类 → 同 description 的其他 pending tx 全部自动归类。③ transactions 列表行加内联改分类（点 category 标弹下拉），改完同样触发扩散+学习+ cashflow 重算 |
